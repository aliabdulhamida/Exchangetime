...
export default function HoldingDetailsModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="grid-rows-[auto_minmax(0,1fr)] gap-0 h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] max-w-[calc(100vw-1rem)] overflow-hidden rounded-xl border border-border bg-background sm:h-[82vh] sm:w-full sm:max-w-3xl">
        <DialogHeader className="border-b border-border bg-background px-3 py-3 sm:px-4 sm:py-3.5">
          <div className="pr-10 sm:pr-8">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
              <div className="min-w-0">
                ...
                <DialogDescription className="mt-1 text-[11px] leading-relaxed text-muted-foreground sm:text-xs">
              ...
              <div className="flex items-center justify-between sm:block sm:text-right">
                ...
                <div className={marketQuote?.dayChangePct !== null && (marketQuote?.dayChangePct || 0) >= 0 ? 'text-[11px] text-emerald-400 sm:text-xs' : 'text-[11px] text-rose-400 sm:text-xs'}>
        ...
        <div className="min-h-0 space-y-2.5 overflow-y-auto px-3 py-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] sm:max-h-[74vh] sm:space-y-3 sm:px-4 sm:py-3.5">
              {holding ? (
                    ...
                    <div
                      className={
                        holding.unrealizedPnl >= 0
                          ...
                          : 'text-base font-semibold text-rose-400 sm:text-lg'
                      }
                    >
                      {formatMoney(holding.unrealizedPnl, baseCurrency, 0, 0)}
                    </div>
                    <div className="text-[11px] text-muted-foreground">
                      {formatPercent(holding.unrealizedPct, 2)}
                    </div>
                  </div>
                  <div className="rounded-xl border border-border bg-background p-2.5 sm:p-3">
                    <div className="text-[11px] text-muted-foreground">Open Cost Basis</div>
                    <div className="text-base font-semibold sm:text-lg">
                      {formatMoney(holding.openCostBasis, baseCurrency, 0, 0)}
                    </div>
                  </div>
                </div>
              ) : null}
              ...
              <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
                <div className="rounded-xl border border-border bg-background p-2.5 text-[11px] sm:p-3 sm:text-xs">
                  ...
                  {holding ? (
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className={holding.realizedPnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className={holding.totalContribution >= 0 ? 'text-emerald-400' : 'text-rose-400'}>
                ...
                <div className="rounded-xl border border-border bg-background p-2.5 text-[11px] sm:p-3 sm:text-xs">
                  ...
                  {error ? (
                    <div className="mb-2 rounded-lg border border-rose-400/35 bg-rose-500/10 px-2.5 py-1.5 text-[11px] text-rose-200">
                  ...
                  {loading && !marketQuote ? (
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={
                            marketQuote?.dayChangePct !== null && (marketQuote?.dayChangePct || 0) >= 0
                              ? 'text-emerald-400'
                              : 'text-rose-400'
                          }
                        >
                ...
                <div className="rounded-xl border border-border bg-background p-2.5 text-[11px] sm:p-3 sm:text-xs">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {websiteHref ? (
                        <a
                          className="max-w-[60%] truncate text-right text-foreground hover:text-muted-foreground"
                          href={websiteHref}
                          target="_blank"
                            ...
                            return (
                              <div className="rounded-lg border border-border bg-background px-3 py-2 text-xs">
                                ...
                                <div className="font-semibold">
                                  {formatMoney(point.value, quoteCurrency, 2, 2)}
                                </div>
                              </div>
                            );
                          }}
                ...
                {transactions.length ? (
                  <div className="space-y-2 sm:max-h-44 sm:overflow-y-auto">
                    {transactions.slice(0, 12).map((tx) => (
                      <div
                        key={tx.id}
                        className="rounded-lg border border-border bg-background px-2.5 py-2 text-[10px] sm:text-[11px]"
                      >
